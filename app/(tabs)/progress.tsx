      ...
      {category ? <>
        ...
        <View style={styles.topicList}>{selectedUnits.map((unit, index) => <View key={unit.id} style={styles.topicRow}>
          ...
          <Pressable onPress={() => router.push({ pathname: '/lesson', params: { id: unit.id } })} style={({ pressed }) => [styles.topic, pressed && styles.pressed]}>
            <View style={styles.topicText}><Text style={styles.topicTitle}>{unit.title}</Text><Text style={styles.topicSummary}>{unit.summary}</Text><Text style={styles.topicMeta}>{unit.cards.length} activities · {unit.level}</Text></View><MaterialCommunityIcons name="chevron-right" size={22} color={London.tube} />
          </Pressable>
        </View>)}</View>
      </> : <>
        ...
        <View style={styles.categoryList}>{STUDY_CATEGORIES.map((item) => {
          ...
          return <Pressable key={item.id} onPress={() => setCategory(item)} style={({ pressed }) => [styles.category, pressed && styles.pressed]}><View style={styles.categoryIcon}><MaterialCommunityIcons name={item.icon} size={25} color={London.white} /></View><View style={styles.categoryText}><Text style={styles.categoryTitle}>{item.title}</Text><Text style={styles.categorySubtitle}>{item.subtitle}</Text><Text style={styles.categoryMeta}>{count} topics</Text></View><MaterialCommunityIcons name="arrow-right" size={21} color={London.tube} /></Pressable>;
        })}</View>
        <View style={styles.tip}><MaterialCommunityIcons name="lightbulb-on-outline" size={20} color={London.gold} /><Text style={styles.tipText}>For quick variety, return to Practice and start a mixed session: multiple choice, writing tasks and visual flashcards are shuffled together.</Text></View>
      </>}
    </View>
  </ScrollView></View>;
}
